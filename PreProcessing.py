...
def add_noise(add_pos_noise,win_name,contrast,noise_scale):
    ...
    if add_pos_noise:
        ...
        print(["./add_noise2",file_to_read,file_to_write_1,"-sca",noise_scale[0],"-idum","-"+str(rand_seed)])
        subprocess.run(["./add_noise2",file_to_read,file_to_write_1,"-sca",noise_scale[0],"-idum","-"+str(rand_seed)])
        rand_seed = 130000+rand.randint(1,1000)
        subprocess.run(["./add_noise2",file_to_read,file_to_write_2,"-sca",noise_scale[0],"-idum","-"+str(rand_seed)])
    else:
